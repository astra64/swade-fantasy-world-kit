...
1. **v0.6.0: Character Manager — Unified Character Creation & Advancement Tools** — Build SWADE character creation support plus guided advancement automation for hassle-free campaign progression.
   ...
   - **Status (v0.6.0 Phase 1)**: Concept, Ancestry, Hindrances, Traits (Attributes & Skills), and Edges tabs complete and tested in-Foundry. Template modularized into Handlebars partials for maintainability. Ancestry bonuses automatically detected from ability effects with lock minimums. Skill and attribute descriptions displayed on hover, with short one-line attribute tips. Edges and Hindrances display "Granted by X" child items (e.g. Arcane Background grants) the same way Ancestry displays ancestral abilities, all collapsible. Drag-and-drop works on all four tabs and accepts any item — from the configured compendium or not — with a name-matching fallback when it isn't. Hindrance perk allocations (Raise Attribute, Gain Skill Point, Choose an Edge) all correctly add bonus points to their respective budgets. Ancestry abilities that grant a free Edge (e.g. Humans' "Adaptable") add to the edge point budget too, matched by name against a configurable world setting (`bonusEdgePointAbilityNames`) rather than compendium-specific tagging, so it works with any setting's Human-equivalent ancestry. Remaining: Gear, Summary tabs. `getData()` prepopulates Name/Archetype/Concept/Ancestry/Attributes/Skills/Edges/Hindrances from an existing actor on open, reading Skills/Edges/Hindrances from `actor.itemTypes.skill/edge/hindrance` (embedded Items) matched to compendium entries by name — prepopulation is implemented per-tab as each tab is built, ahead of Save. **Save is still a scaffold**: `_createActor()` writes Skills/Edges/Hindrances to `actor.system.skills/edges/hindrances`, which isn't where SWADE stores them, and is intentionally deferred — real persistence (mirroring the ancestry embedded-item pattern, plus setting a `compendiumUuid` flag like Ancestry does so future prepopulation can prefer it over name-matching) is planned as part of the Summary tab (Milestone 3, Tab 8), not something to fix mid-tab-development.
   ...
   - **Testing:** [docs/v0.6.0/TESTING.md](docs/v0.6.0/TESTING.md) — Comprehensive test cases aligned with new design.
