...
export class BaselineModulesManager extends FormApplication {
  ...
  _showApplyResult(enabledNow, disabledNow, alreadyEnabled, missing, autoIncludedDependencies) {
    let message = `Preset applied: +${enabledNow.length} enabled, -${disabledNow.length} disabled.`;
    ui.notifications?.info(`SWADE Fantasy World Kit: ${message}`);
  }
  ...
  async _updateObject(_event, formData) {
    ...
    if (action === "save") {
      ...
      ui.notifications?.info("SWADE Fantasy World Kit: Preset saved.");
      ...
      await setActivePresetModuleIds(normalizedIds);
      await updateTitleCache("baselineModuleTitles", normalizedIds);
      ui.notifications?.info("SWADE Fantasy World Kit: Preset saved. Applying to world...");
