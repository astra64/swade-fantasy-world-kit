...
- **Character Manager v0.6.0 (Phase 1 In Progress)**:
  ...
  - **Gear Tab: Starting Equipment vs Gear Management Mode**: a toggle at the top of the Gear tab switches its footer between the creation-time shopping budget (remaining/total funds, override) and a plain current-currency readout for an already-established character — avoids showing a stale "starting funds" budget once a character has moved past character creation. Defaults to Starting Equipment for a fresh actor, Gear Management for one with existing advances or gear; remembered per-actor once set.
