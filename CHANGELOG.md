...
- **Character Manager v0.6.0 (Phase 1 In Progress)**:
  ...
  - **Gear Tab: Starting Equipment vs Gear Management Mode**: a compact toggle inline in the Gear tab's guidance box switches its footer between the creation-time shopping budget (remaining/total funds, override) and a plain current-currency readout for an already-established character — avoids showing a stale "starting funds" budget once a character has moved past character creation. Defaults to Starting Equipment for a fresh actor, Gear Management for one with existing advances or gear; remembered per-actor once set. Both modes update the actor's real currency on Save, just differently: Starting Equipment sets currency to the creation budget minus what's spent (fixing a bug where a freshly-created actor's SWADE-seeded starting money got double-counted); Gear Management debits/credits only the change in gear cost since the tab was opened, so buying or returning gear adjusts existing wealth like a normal shopping trip without disturbing money the actor already had. **Revised again:** currency now only changes on Save if an explicit "Apply to currency on Save" checkbox in the footer is checked (unchecked by default) — the checkbox sits next to a live preview of the exact effect ("sets currency to 300" / "charges 200 Silver" / "refunds 50 Silver"), so nothing about wealth is silent or automatic in either mode. This replaced an earlier confirmation-dialog approach, since requiring explicit opt-in removes the risk that dialog existed to catch in the first place.
