...
- **Character Manager v0.6.0 (Phase 1 In Progress)**:
  ...
  - **Gear Tab: Moved Out of the Main Tab Bar**: Gear no longer has its own always-visible tab-bar button — it's reachable only via a "Manage Gear" shortcut inline in the Summary tab's guidance box, since its shopping/currency concerns read as distinct from the rest of character build planning. The tab itself, its handler, and Save behavior are unchanged; only its nav-bar visibility changed.
