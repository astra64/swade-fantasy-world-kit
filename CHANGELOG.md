...
- **Character Manager v0.6.0 (Phase 1 In Progress)**:
  ...
  - **Gear Tab Customization-Safe Saves**: an item already on the actor is never deleted and recreated on Save — only its quantity is patched in place — so manual tweaks to a gear/weapon/armor item always survive re-saving the character
