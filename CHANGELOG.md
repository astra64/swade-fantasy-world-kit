...
- **Character Manager v0.6.0 (Phase 1 In Progress)**:
  ...
  - **Ancestry Tab**: Searchable dropdown selection from Fantasy Companion ancestries with compendium browser integration. Save now reconciles the actor's ancestry item instead of unconditionally deleting and recreating it, same principle as the Skills/Edges/Hindrances/Gear tabs — an unchanged selection is left completely untouched, preserving any customization (renamed item, edited description), and only an actual change of ancestry triggers a delete+create. **Bug fixed**: saving with an ancestry that wasn't from the module's configured compendium (e.g. a core-created or homebrew ancestry) used to delete it from the actor with nothing put back in its place, because the old logic always deleted the existing item first, then re-resolved the selection by UUID to recreate it — for a non-compendium ancestry that UUID pointed at the very item just deleted, so nothing came back.
  - **Hindrances Tab**: Major/Minor toggle (no point cap on hindrance selection itself — only the perk slots hindrance points grant are limited), hindrance selection with compendium browser, perk point allocation system; changing a perk allocation no longer scrolls the tab to the top (it re-anchors to the slot you just changed, same fix as other toggles already used). **Bug fixed**: with an odd number of perk points (e.g. 3), selecting a 2-point perk in one dropdown used to make the dropdown for the remaining point vanish entirely instead of staying available — two separate, conflicting "hide the next slot" mechanisms (one in slot generation, one in the template) were both trying to collapse slots based on the *previous* slot's selection rather than how many points it actually consumed; replaced with a single calculation that sizes each slot from real remaining points. **Bug fixed**: the Summary tab's Hindrances row showed an over/under-budget warning color and an "/4" target like Attributes/Skills/Edges, but hindrance selection has no target to hit — any number is valid — so it's now shown as a plain count, e.g. "Hindrances (2)".
  - **Traits Tab (Attributes & Skills)**: Full attribute and skill selection with die-based progression, free core skills, ancestry attribute bonuses with lock minimums, and hover tooltips for skill descriptions. **Bug fixed**: skill dice were also read from the actor's derived (post-Active-Effects) data like the attribute bug above — a temporary effect targeting a skill's die would show up on open and could get permanently baked into the item as its new base on Save. Skills are now read from source data, same fix as attributes.
  ...
  - **Gear Tab Starting Funds Formula**: starting funds now come from a real formula (base setting × a matched Rich/Filthy Rich-type edge multiplier, plus the Hindrances tab's Extra Funds perk bonus) instead of an unconditional doubling, with a manual override available. A pinned footer on the Gear tab shows remaining/total funds and the override toggle. All of this is automatically hidden for tables using SWADE's Wealth Die or no-currency setting rules.
  - **Gear Tab: Starting Equipment vs Gear Management Mode, with Explicit Currency Changes**: a compact toggle inline in the Gear tab's guidance box switches its footer between the creation-time shopping budget (remaining/total funds, override) and a plain current-currency readout for an already-established character — avoids showing a stale "starting funds" budget once a character has moved past character creation. Defaults to Starting Equipment for a fresh actor, Gear Management for one with existing advances or gear; remembered per-actor once set. Currency itself only ever changes on Save if an "Apply to currency on Save" checkbox in the footer is explicitly checked (unchecked by default), next to a live preview of the exact effect — "sets currency to 300" in Starting mode (creation budget minus spend), or "charges 200 Silver" / "refunds 50 Silver" in Gear Management mode (debits/credits only the change in gear cost since the tab was opened, so existing wealth is never disturbed for gear the actor already owned). No currency change is ever a silent side effect of Save in either mode. Save's existing confirmation dialog (previously just unspent Attribute/Skill/Edge points) now also always states plainly what will happen to currency — including explicitly confirming "currency will not change" when the checkbox is unchecked — as one combined last check before anything is written.
  - **Gear Tab: Encumbrance Tracking**: the pinned footer now always shows Carried/Max weight (turning red when over), regardless of the currency setting. Max carry capacity mirrors SWADE's own carry-capacity formula, based on Strength die and the actor's live encumbrance-step bonuses (e.g. Packrat, racial size). Known approximation: doesn't track Consumable-type items or equip-status (stored vs. carried), so it can drift from what the actor's real sheet shows.
  - **Gear Tab: Renamed and Moved to Last**: renamed "Gear" to "Manage Gear" and moved it after both Summary and Advancement (previously between Edges and Summary), signaling it's a distinct, later-stage concern (shopping/currency) rather than part of the core build flow. Still has its own tab-bar button; also reachable via a "Manage Gear" shortcut button on the Summary tab, which now shows a small "Don't forget to select your starting gear" prompt when the Gear tab is in Starting Equipment mode.
  - **Gear Tab: Fixed duplicate items on re-adding existing gear**: picking an item you already own again (via the search dropdown, or drag-drop) now correctly bumps its quantity instead of creating a second, separate item on the actor. The bug: an already-saved item and a freshly-picked compendium entry for the "same" item live under two different internal IDs, so the old duplicate check (which compared IDs) missed the match — this is now name-based instead, matching how the feature was always described.
  - **Gear Tab: Excludes SWADE Fantasy Currencies Coin Items**: gear-type items flagged by the separate "SWADE Fantasy Currencies" module (coin items it manages via its own currency UI) no longer show up in, or get deleted by, the Gear tab's list — they're left entirely to that module's own sync between coins and currency.
  - **Advancement Tab**: Record post-creation Advances (New Edge, Two Skills, One Skill, Attribute, Hindrance Buyoff), grouped by Rank as you take them (Novice covers the first 3, then 4 apiece per Rank after, matching SWADE's own banding). Reads and writes SWADE's real `system.advances.list` directly rather than a separate ledger, and each entry adds matching bonus points into the Edges/Traits/Hindrances tabs' existing budgets — reusing their pickers instead of duplicating them. A Hindrance Buyoff advance grants a bonus perk-point slot capped alongside the normal 4-point perk cap, so it only ever restores a point lost to the buyoff rather than exceeding the cap. Each row also has a "Planned" toggle mirroring SWADE's own Advances tab — a planned advance displays but doesn't count toward Rank or budgets until unchecked. An Attribute advance beyond the once-per-Rank guideline surfaces a dedicated warning, informational only like every other budget check in this tool.
  ...
  - **Perk Allocation UI**: Dropdown system to allocate hindrance trade-off points (skill points, attribute boosts, edges, bonus currency) — all four options now actually affect their respective budgets (previously only the Edge option did anything). **Bugs fixed**: (1) perk allocations weren't persisted anywhere, so they silently reset every time Character Manager was reopened — now saved as a `perkPointAllocations` actor flag; (2) with an odd number of perk points (e.g. 3), selecting a 2-point perk made the dropdown for the remaining point disappear entirely instead of staying available for a 1-point choice.
  - **Hindrances Tab: Edge-Granted Hindrances No Longer Leak Into the Tab**: a hindrance granted as a child item by a selected edge (e.g. an Arcane Background's downside) no longer appears as an independent "Selected Hindrance" card, no longer grants its own perk points, and no longer shows a confusing "further grants" section of its own — it wasn't an independent player choice, so it's excluded from the tab's display and point calculations (matched by name against the edge's own `system.grants` list, since the two live under different internal IDs). Its real mechanical effects (Active Effects on Pace/Toughness, etc.) still apply — only the tab display and hindrance-point accounting changed.
  ...
  - **Ancestry Bonuses**: Automatically detects and displays attribute bonuses from ancestry abilities with visual indicator and lock minimums. **Bug fixed**: an ancestry-granted attribute die (e.g. Elf's boost to Agility) was being counted against the player's 5 attribute points instead of being a free floor — attribute points spent are now only the steps taken *above* whatever the ancestry already grants for nothing. **Bug fixed**: Character Manager was reading the actor's derived attribute values (post-Active-Effects) rather than its permanent base — a temporary condition active at open time (e.g. an injury lowering Agility) showed up as the character's die on the Traits tab and, if the tab was saved, would have permanently overwritten the actor's real base with that temporarily-lowered value. Attributes are now read from source data, with the ancestry's own floor re-applied on top (so the ancestry bonus still displays correctly) while any unrelated temporary effect is excluded. **Bug fixed**: for an ancestry whose attribute bonus comes from an additive Active Effect, Save was writing the floor-boosted die back as the attribute's base — since the actor's own effect then added its bonus again on top, this doubled the ancestry's attribute bonus every time the character was saved. Save now writes back only the die below the ancestry's ADD-mode bonus, leaving the actor's own effect as the sole source of that portion, and reopening Character Manager correctly reconstructs the full die (including any manual raise above the ancestry's floor) by adding that same bonus back on top of source. **Bug fixed**: that reconstruction re-added the ancestry's bonus on every render instead of once, so the Traits tab's die ratcheted up a step each time it re-rendered — clicking a die button appeared to select one step higher than clicked, and eventually nothing below the inflated value could be selected. Each attribute now tracks the bonus amount it's already been credited for and only adjusts it when the ancestry itself changes, leaving renders in between (and the player's own clicks) alone.
  - **Ancestry Tab: Ability Bonus Choice**: for ancestral abilities whose compendium entry carries no mechanical effects of its own (e.g. Half-Elf's Heritage: pick either a free Edge or a free Attribute point), a dropdown on the ability's card lets the player choose a bonus (Edge/Attribute/Skill point), which flows into the same budgets as every other bonus source. Only shown for abilities matching a configurable, comma-separated world setting ("Bonus-Choice Ancestral Ability Names", defaults to "Half-Elves-Heritage") — same name-matching approach as the existing Free-Edge setting, so other settings' similar "choose one" heritage abilities can be added without any code change. Persisted as an array of `{uuid, choice}` entries rather than an object keyed by uuid — a compendium uuid contains periods, and Foundry's flag storage recursively merges objects treating "." as a path separator, which silently corrupts a plain object keyed by uuid.
  - **Ancestry Tab: Manual Adjustments**: a collapsible section for adding or subtracting points from any budget (Attribute/Skill/Edge/Hindrance-Perk) via +/- steppers, with a freeform note — an escape hatch for setting rules or ancestral quirks this tool has no built-in support for. Non-zero adjustments show as a note on the Summary tab.
...
- Removed the skeletal, non-functional standalone `AdvancementManager.js`/`advancement-manager.hbs` (used a flat `system.experience`/`system.advances` schema that doesn't exist in SWADE) — superseded by the new Advancement tab above
- Default Character Manager window height increased (500 → 700) so the added Advancement tab and its guidance text need less scrolling on open
- **Unspent-Points Visibility**: the sticky footer now also shows a Perks counter (spent/available, hidden when a character has no hindrance-granted perk points) that highlights red for either over-allocation or leftover unspent points; the Summary tab's Ancestry row now flags any ancestral bonus-choice dropdown (e.g. Half-Elf's Heritage) that hasn't been assigned yet; and the Save confirmation dialog now also warns about unspent perk points and unassigned ancestry bonuses alongside the existing Attribute/Skill/Edge check. The Hindrances tab's perk-allocation dropdown's placeholder option is now itself selectable, so a slot can be cleared back to unassigned instead of being stuck once a choice is made.
