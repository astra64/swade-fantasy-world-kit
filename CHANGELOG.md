...
- **Character Manager v0.6.0 (Phase 1 In Progress)**:
  ...
  - **Hindrances Tab**: Major/Minor toggle (no point cap on hindrance selection itself — only the perk slots hindrance points grant are limited), hindrance selection with compendium browser, perk point allocation system; changing a perk allocation no longer scrolls the tab to the top (it re-anchors to the slot you just changed, same fix as other toggles already used). **Bug fixed**: with an odd number of perk points (e.g. 3), selecting a 2-point perk in one dropdown used to make the dropdown for the remaining point vanish entirely instead of staying available — two separate, conflicting "hide the next slot" mechanisms (one in slot generation, one in the template) were both trying to collapse slots based on the *previous* slot's selection rather than how many points it actually consumed; replaced with a single calculation that sizes each slot from real remaining points.
