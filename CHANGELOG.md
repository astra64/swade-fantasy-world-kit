...
- **Character Manager v0.6.0 (Phase 1 In Progress)**:
  ...
  - **Advancement Tab**: Record post-creation Advances (New Edge, Two Skills, One Skill, Attribute, Hindrance Buyoff), grouped by Rank as you take them (Novice covers the first 3, then 4 apiece per Rank after, matching SWADE's own banding). Reads and writes SWADE's real `system.advances.list` directly rather than a separate ledger, and each entry adds matching bonus points into the Edges/Traits/Hindrances tabs' existing budgets — reusing their pickers instead of duplicating them. Each row also has a "Planned" toggle mirroring SWADE's own Advances tab — a planned advance displays but doesn't count toward Rank or budgets until unchecked. An Attribute advance beyond the once-per-Rank guideline surfaces a dedicated warning, informational only like every other budget check in this tool.
